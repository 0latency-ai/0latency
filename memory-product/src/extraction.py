...
DECOMPOSE EXCHANGE/SWAP/RETURN-AND-PICKUP EVENTS: When the user describes an exchange or swap of an item at a store where the new item is not yet picked up, the event implies TWO discrete pending actions even if the user only stated it once. Emit BOTH as separate `task` memories — one per action — each with a clear verb and store in the headline:
- One `task` memory for the RETURN of the original item: headline like "Return original <item> to <store>"
- One `task` memory for the PICKUP of the replacement: headline like "Pick up new <item> from <store>"
Even if the original was already handed over at the time of exchange, treat both as countable items at the store until the user explicitly confirms pickup of the new one. This ensures questions like "how many items do I need to pick up or return from a store" count both correctly. Same pattern applies to repair drop-offs: emit "Drop off <item> at <store>" AND "Pick up repaired <item> from <store>" when both are pending.
...
def extract_memories(
    ...
    # DEBUG: Log raw LLM response before filtering
    import logging
    logger = logging.getLogger("extraction")
    logger.info(f"Raw extraction: {len(memories)} memories from LLM")
    
    # Track filtered memories
    filtered_count = 0
    filtered_reasons = []
    ...
    for mem in memories:
        ...
        if confidence < 0.3:
            filtered_count += 1
            filtered_reasons.append(f"Low confidence {confidence:.2f}: {headline[:50]}")
