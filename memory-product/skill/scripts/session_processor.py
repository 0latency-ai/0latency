...
# --- Configuration ---
# Auto-detect OpenClaw paths or use environment overrides
OPENCLAW_HOME = os.environ.get("OPENCLAW_HOME", os.path.expanduser("~/.openclaw"))
AGENT_ID = os.environ.get("MEMORY_AGENT_ID", "default")

# Build agents config from environment or auto-detect
def _detect_agents():
    """Auto-detect OpenClaw agent sessions."""
    agents = {}
    agents_dir = os.path.join(OPENCLAW_HOME, "agents")
    workspace_dir = os.environ.get("MEMORY_WORKSPACE", os.path.join(OPENCLAW_HOME, "workspace"))
    
    if os.path.isdir(agents_dir):
        # Find main agent
        main_sessions = os.path.join(agents_dir, "main", "sessions")
        if os.path.isdir(main_sessions):
            agents[AGENT_ID] = {
                "session_dir": main_sessions,
                "workspace_dir": workspace_dir,
            }
    
    # Allow additional agents via MEMORY_EXTRA_AGENTS env (JSON)
    extra = os.environ.get("MEMORY_EXTRA_AGENTS", "")
    if extra:
        try:
            agents.update(json.loads(extra))
        except:
            pass
...
STATE_FILE = os.environ.get("MEMORY_STATE_FILE", 
    os.path.join(OPENCLAW_HOME, "workspace", ".memory_processor_state.json"))
LOG_FILE = os.environ.get("MEMORY_LOG_FILE",
    os.path.join(os.path.expanduser("~"), "logs", "memory_processor.log"))
