...
async def extract_endpoint(req: ExtractRequest, tenant: dict = Depends(require_api_key)):
    ...
    try:
        ...
        if not memories:
            ...
            # Inject project/thread metadata from request into each memory dict
            if req.project_id or req.thread_id or req.project_name or req.thread_title:
                for mem in memories:
                    if req.project_id:
...
async def search_memories(
    ...
    try:
        ...
        query = f"""
            ...
            LIMIT %s
        """
        params = (agent_id, tenant["id"], pattern, pattern) + ((project_id,) if project_id else ()) + (limit,)
        rows = _db_execute_rows(query, params, tenant_id=tenant["id"])
