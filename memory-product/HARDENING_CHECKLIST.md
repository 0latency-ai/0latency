...
- [ ] **Rate limiting persistence** — currently in-memory dict, resets on restart. Move to Redis or Postgres-backed counter.
- [ ] **Database connection handling** — no connection pooling, no retry logic, no timeout handling. Every query spawns a subprocess psql call. Move to psycopg2 connection pool.
- [ ] **Error handling** — bare try/except in most endpoints. Need structured error responses, proper logging, no stack traces in production responses.
...
- [ ] **Recall edge cases** — empty memory store, budget=0, extremely long conversation_context, special characters.
