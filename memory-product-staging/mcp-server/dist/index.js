 ...
 */
async function runActiveProfiler(agentId, humanMessage, sessionKey) {
    try {
...
function formatSentinelWarning(sentinel) {
    ...
    for (const finding of sentinel.detected) {
        lines.push(`   • [${finding.confidence?.toUpperCase()}] ${finding.pattern_name}: ${finding.redacted}`);
    }
    lines.push("");
    lines.push(`   Action taken: ${sentinel.action?.toUpperCase()}`);
    if (sentinel.action === "flagged") {
        lines.push("   ℹ️  Content was stored as-is. Consider removing secrets from your memory content.");
        lines.push("   💡 To auto-redact secrets, ask your admin to set sentinel_mode to 'redact'.");
    }
    else if (sentinel.action === "redacted") {
        lines.push("   ✅ Secrets were automatically redacted before storage.");
    }
    lines.push("");
    return lines.join("\n");
}
function appendSentinelWarning(baseText, apiResult) {
...
// ── memory_add ──────────────────────────────────────────────────────────────
server.tool("memory_add", "Extract and store memories from a conversation turn. Provide the human message, agent response, and an agent_id to namespace the memories.", {
...
server.tool("remember", "Store a piece of information to remember across conversations. Use this whenever the user asks you to remember something, or when important facts come up worth preserving.", {
...
// ── seed_memories ───────────────────────────────────────────────────────────
server.tool("seed_memories", "Seed memories directly from raw facts, bypassing the extraction pipeline. Use this to bulk-load known facts, preferences, or context into an agent's memory without conversation-format input.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
    facts: z
        .array(z.object({
...
server.tool("load_memory_pack", "Load a curated memory pack to give your agent instant domain expertise. Available packs: saas-founder, typescript-dev, python-dev, claude-power-user. Use 'list' as pack_name to see all options.", {
...
// ── memory_recall ───────────────────────────────────────────────────────────
server.tool("memory_recall", "Recall relevant memories given a conversation context. Returns a formatted context block ready to inject into a prompt.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_feedback ────────────────────────────────────────────────────────
server.tool("memory_feedback", "Submit feedback on recalled memories. Helps 0Latency learn which memories are useful vs ignored. This powers self-improving importance scores.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_search ───────────────────────────────────────────────────────────
server.tool("memory_search", "Search memories by text query. Returns matching memories ranked by relevance.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_list ─────────────────────────────────────────────────────────────
server.tool("memory_list", "List stored memories with optional filters. Supports pagination via limit/offset.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_delete ───────────────────────────────────────────────────────────
server.tool("memory_delete", "Delete a specific memory by its ID.", {
...
// ── import_document ─────────────────────────────────────────────────────────
server.tool("import_document", "Import a large text document (project brief, wiki page, documentation, etc.) and extract memories from it. Content is automatically chunked and processed through the extraction pipeline.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── import_conversation ─────────────────────────────────────────────────────
server.tool("import_conversation", "Import a conversation export (e.g. from Claude Desktop or ChatGPT) and extract memories from each turn pair. Provide the conversation as an array of {role, content} objects.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
    conversation: z
        .array(z.object({
...
// ── memory_graph ────────────────────────────────────────────────────────────
server.tool("memory_graph", "Query the knowledge graph. List entities, explore an entity's relationships, or find the path between two entities.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
    action: z
    ...
    switch (action) {
        ...
        case "entity_detail":
            if (!entity)
                throw new Error("entity is required for entity_detail");
            result = await api({
                path: "/graph/entity",
        ...
        case "entity_memories":
            if (!entity)
                throw new Error("entity is required for entity_memories");
            result = await api({
        ...
        case "find_path":
            if (!source || !target)
                throw new Error("source and target are required for find_path");
            result = await api({
                path: "/graph/path",
...
server.tool("memory_graph_traverse", "Get related memories via graph relationships. Finds memories connected to a starting memory through semantic relationships. Requires Pro or Scale tier.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_entities ─────────────────────────────────────────────────────────
server.tool("memory_entities", "List all entities extracted from an agent's memories, with mention counts and type info. Requires Pro or Scale tier.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
server.tool("memory_sentiment_summary", "Get aggregate sentiment statistics for an agent's memories. Returns positive/negative/neutral counts, averages, and confidence stats. Requires Pro or Scale tier.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
}, async ({ agent_id }) => {
    const result = await api({
...
// ── memory_by_entity ────────────────────────────────────────────────────────
server.tool("memory_by_entity", "Find all memories mentioning a specific entity. Requires Pro or Scale tier.", {
    agent_id: z.string().min(1).max(128).default("default").describe("Namespace (use 'default' unless user specifies)"),
...
// ── memory_history ──────────────────────────────────────────────────────────
server.tool("memory_history", "Get the full version history for a specific memory. Shows how the memory evolved over time, including what changed and why.", {
